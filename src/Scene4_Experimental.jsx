...
// Main component
export default function Scene4_PerspectivePuzzle() {
  console.log("RENDERING Scene4_PerspectivePuzzle COMPONENT");
  ...
  // Check for existing bone count in sessionStorage when component mounts
  useEffect(() => {
    const storedBoneCount = sessionStorage.getItem('totalCollectedBones');
    if (storedBoneCount) {
      console.log("Retrieved stored bone count:", storedBoneCount);
      setTotalBoneCount(parseInt(storedBoneCount));
    }
      ...
      timerInterval = setInterval(() => {
        setTimeRemaining(prev => {
          if (prev <= 1) {
            ...
            setFinalScore(inventory.length);
            return 0;
          }
          return prev - 1;
        });
  ...
  // Update finalScore and totalBoneCount when game ends
  useEffect(() => {
    if (gameState === 'finished') {
      setFinalScore(inventory.length);
      
      // Calculate new total including current inventory
      const previousCount = parseInt(sessionStorage.getItem('totalCollectedBones') || '0');
      const newTotal = previousCount + inventory.length;
  ...
  const startGame = () => {
    ...
    setTimeRemaining(60);
    setInventory([]);
    // Reset all bones to uncollected state
    setBones(prev => prev.map(bone => ({
      ...bone,
      collectProgress: 0,
      collected: false
    })));
  ...
  // Function to navigate to Scene5 
  const navigateToScene5 = () => {
    // Save the current bone count to sessionStorage
    const previousCount = parseInt(sessionStorage.getItem('totalCollectedBones') || '0');
    const newTotal = previousCount + inventory.length;
    sessionStorage.setItem('totalCollectedBones', newTotal.toString());
    console.log(`Saved total bone count: ${newTotal} (previous: ${previousCount} + current: ${inventory.length})`);
    
    // Save game state and timer to sessionStorage for persistence
    sessionStorage.setItem('gameState', 'playing');
    sessionStorage.setItem('timeRemaining', timeRemaining.toString());
    console.log(`Saved game state: playing, time remaining: ${timeRemaining}`);
    
    // Navigate to Scene 5
    try {
      // Try to find the button in the parent App component and click it
      const buttons = document.querySelectorAll('button');
      const scene5Button = Array.from(buttons).find(button => 
        button.textContent.includes('Scene 5') || button.textContent.includes('scene5')
      );
      
      if (scene5Button) {
        console.log('Found Scene 5 button, clicking it');
        scene5Button.click();
      } else {
        // Use the state-based approach
        console.log('No Scene 5 button found, trying window.setCurrentScene');
        if (window.parent && window.parent.setCurrentScene) {
          window.parent.setCurrentScene('scene5');
        } else if (window.setCurrentScene) {
          window.setCurrentScene('scene5');
        } else {
          console.log('Falling back to hash navigation');
          window.location.hash = 'scene5';
        }
      }
    } catch (e) {
      console.error('Error navigating to Scene 5:', e);
      // Emergency fallback
      window.location.hash = 'scene5';
    }
  };
  ...
  return (
    <div style={{ width: '100%', height: '100vh' }}>
      <Canvas shadows camera={{ position: [35, 6, 0], fov: 50 }}>
        <StarrySky />
        <Scene 
          bones={bones} 
          setBones={setBones} 
          ...
          navigateToScene5={navigateToScene5}
        />
      </Canvas>
      ...
      {gameState === 'playing' && (
        <div style={{
          ...
          fontWeight: 'bold',
          textAlign: 'center',
          boxShadow: timeRemaining <= 10 ? '0 0 10px #ff3333' : 'none',
      ...
      {portalActive && (
        <div style={{
          ...
          <p>Press [E] to enter the Alveolar Harbor</p>
          {gameState === 'playing' && (
      ...
      {gameState === 'ready' && (
        <div style={{
          ...
          <h2 style={{ color: '#ff66cc', marginTop: 0 }}>Bone Collection Challenge</h2>
          <p>Collect as many bone fragments as possible in 60 seconds!</p>
          <button
            onClick={startGame}
            style={{
              background: '#ff66cc',
      ...
      {gameState === 'finished' && (
        <div style={{
          ...
          <h2 style={{ color: '#ff66cc', marginTop: 0 }}>Time's Up!</h2>
          <p>You collected <span style={{ color: '#ff66cc', fontWeight: 'bold' }}>{finalScore} bone fragments</span> in this area!</p>
          ...
          <div style={{ 
            ...
            <p style={{ margin: '0' }}>Total bones collected:</p>
            <p style={{ 
              fontSize: '24px', 
              fontWeight: 'bold', 
              margin: '5px 0', 
          ...
          <p style={{ fontSize: '14px', opacity: 0.8, marginBottom: '15px' }}>
            Continue your journey to find more bone fragments
          </p>
      ...
      <div style={{
        ...
        {inventory.length > 0 ? 
          ...
          : <div>No bones collected yet</div>
        }
      </div>
...
// Scene component
function Scene({ bones, setBones, characterPosition, setCharacterPosition, inventory, setInventory, showInstructions, teleportTarget, setTeleportTarget, resetCamera, gameState, portalActive, setPortalActive, navigateToScene5 }) {
  ...
  const initialCameraPosition = useMemo(() => ({
    position: new THREE.Vector3(35, 6, 0),
  ...
  }, [setBones, setInventory, setPortalActive, gameState, scene, navigateToScene5]);
  ...
  return (
    <>
      ...
      {/* Lighting */}
      <ambientLight intensity={0.2} color="#ff66cc" />
      <directionalLight 
        position={[10, 10, 5]} 
        intensity={0.5} 
        castShadow 
      ...
      <pointLight position={[0, 3, 0]} intensity={1} color="#ff66aa" distance={10} />
      <pointLight position={[5, 2, 5]} intensity={0.5} color="#cc66ff" distance={8} />
      <pointLight position={[-5, 2, -5]} intensity={0.5} color="#ff99cc" distance={8} />
  ...
  return (
    <Html position={[0, 2, 0]} center>
      <div style={{
        ...
        <h3 style={{ margin: '0 0 10px 0' }}>Bone Collection Challenge</h3>
        <p>Use WASD or arrow keys to move</p>
        <p>Click anywhere to teleport</p>
        <p>Stand near bones to collect them</p>
        <p>Collect as many bones as possible before time runs out!</p>
        <p style={{ color: '#ff66cc', fontWeight: 'bold' }}>You have 60 seconds when the game starts</p>
      </div>
    </Html>
  );
}
